    ...
    std::unique_ptr<QProcess> launch(const LaunchConfig &config) override {
        ...
        if (!process->waitForStarted(3000)) {
            LOG_ERROR(QString("Failed to start game: %1 - %2")
                          .arg(config.gamePath, process->errorString()));
            return nullptr;
        }
...
RunnerManager::RunnerManager(QObject *parent) : QObject(parent) { discoverRunners(); }
    ...
    if (hostArch == Architecture::ARM64) {
        ...
        if (!box64.isEmpty()) {
            ...
            registerRunner(std::make_unique<WrapperRunner>(
                "Box64", box64, Platform::Linux, Architecture::ARM64, Architecture::X86_64, true));
        }
        ...
        if (!fex.isEmpty()) {
            ...
            registerRunner(std::make_unique<WrapperRunner>(
                "FEX", fex, Platform::Linux, Architecture::ARM64, Architecture::X86_64, true));
        }
        ...
        if (!qemu.isEmpty()) {
            ...
            registerRunner(std::make_unique<WrapperRunner>(
                "QEMU", qemu, Platform::Linux, Architecture::ARM64, Architecture::X86_64, true));
        }
    }
#endif
...
Runner *RunnerManager::findBestRunner(const LaunchConfig &config) {
    ...
    if (best) {
        LOG_INFO(QString("Auto-selected runner: %1 (score: %2)").arg(best->name()).arg(bestScore));
    }
