...
Session::Session(QObject *parent) : QObject(parent), authenticated_(false) { loadSession(); }

Session::~Session() = default;

void Session::loginWithPassword(const QString &username, const QString &password,
                                AuthCallback callback) {
    QTimer::singleShot(
        100, this, [this, username, password, callback = std::move(callback)]() mutable {
            ...
            const QString CLIENT_SECRET = qEnvironmentVariable(
                "GOG_CLIENT_SECRET", "9d85bc8330b3df6d97c98e309705a47ddbd299ca");

            // GOG API uses GET with URL parameters, not POST with JSON
            QString url = QString("https://auth.gog.com/"
                                  "token?client_id=%1&client_secret=%2&grant_type=password&"
            ...
            client->get(url, [this, callback = std::move(callback)](
                ...
                if (!result.isOk()) {
                    ...
                    if (errorMsg.contains("timeout") || errorMsg.toLower().contains("timed out")) {
                        callback(util::Result<AuthTokens>::error(
                        ...
                        callback(
                            util::Result<AuthTokens>::error("Login failed. Please try again."));
                    }
                    return;
                }
                ...
                if (json.contains("error")) {
                    ...
                    if (errorCode == "invalid_grant" || errorCode == "invalid_client" ||
                        ...
                        callback(
                            util::Result<AuthTokens>::error("Invalid request. Please try again."));
...
void Session::loginWithAuthCode(const QString &authCode, AuthCallback callback) {
    QTimer::singleShot(100, this, [this, authCode, callback = std::move(callback)]() mutable {
        ...
        QString url = QString("https://auth.gog.com/"
                              "token?client_id=%1&client_secret=%2&grant_type=authorization_code&"
                              "code=%3&redirect_uri=%4")
                          .arg(QString(QUrl::toPercentEncoding(CLIENT_ID)),
                               QString(QUrl::toPercentEncoding(CLIENT_SECRET)),
        ...
        client->get(url, [this, callback = std::move(callback)](
            ...
            if (!result.isOk()) {
                ...
                if (errorMsg.contains("timeout") || errorMsg.toLower().contains("timed out")) {
                    callback(
                        util::Result<AuthTokens>::error("Connection timeout. Please try again."));
                } else if (errorMsg.contains("network") || errorMsg.contains("connection") ||
                           errorMsg.contains("host") || errorMsg.contains("resolve")) {
                    callback(util::Result<AuthTokens>::error(
                        "Network error. Please check your internet connection."));
            ...
            if (response.statusCode >= 400) {
                callback(
                    util::Result<AuthTokens>::error("Authorization failed. Please try again."));
                return;
            }
...
void Session::refreshToken(AuthCallback callback) {
    QTimer::singleShot(100, this, [this, callback = std::move(callback)]() mutable {
        ...
        const QString CLIENT_SECRET =
            qEnvironmentVariable("GOG_CLIENT_SECRET", "9d85bc8330b3df6d97c98e309705a47ddbd299ca");

        // GOG API uses GET with URL parameters, not POST with JSON
        QString url =
            QString("https://auth.gog.com/"
                    "token?client_id=%1&client_secret=%2&grant_type=refresh_token&refresh_token=%3")
                .arg(QString(QUrl::toPercentEncoding(CLIENT_ID)),
                     QString(QUrl::toPercentEncoding(CLIENT_SECRET)),
        ...
        client->get(url, [this, callback = std::move(callback)](
            ...
            if (result) {
                ...
                newTokens.expiresAt =
                    QDateTime::currentDateTime().addSecs(json["expires_in"].toInt());
                setTokens(newTokens);
...
void Session::fetchUserInfo(UserCallback callback) {
    ...
    client->request(req, [callback = std::move(callback)](
                             util::Result<net::HttpClient::Response> result) mutable {
        if (!result.isOk()) {
...
QString Session::getSecureStoragePath() const { return util::Config::instance().sessionFilePath(); }
