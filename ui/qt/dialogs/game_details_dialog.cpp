...
GameDetailsDialog::GameDetailsDialog(const api::GameInfo &game,
                                     ...
                                     runners::RunnerManager *runnerManager, QWidget *parent)
    : QDialog(parent), game_(game), libraryService_(libraryService), runnerManager_(runnerManager) {
    ...
    runnerExecutableEdit_->setPlaceholderText(
        tr("Optional: full path to wrapper (overrides auto-detected path)"));

    runnerArgsEdit_ = new QPlainTextEdit(compatBox);
    runnerArgsEdit_->setPlaceholderText(
        tr("Runner arguments (one per line)\nExample: --some-flag"));
    ...
    envEdit_->setPlaceholderText(tr("Environment variables (KEY=VALUE, one per line)\nCommon: "
                                    "DXVK_HUD=fps, VKD3D_CONFIG=..., PROTON_LOG=1"));
...
void GameDetailsDialog::onClose() { accept(); }
...
void GameDetailsDialog::launchWinecfg() {
    ...
    if (!game_.runnerExecutable.isEmpty()) {
        env << "WINEPREFIX=" + QStandardPaths::writableLocation(QStandardPaths::HomeLocation) +
                   "/.wine";
    }
    ...
    if (!ok) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Failed to launch winecfg. Make sure Wine is installed."));
    }
}
...
void GameDetailsDialog::launchProtontricks() {
    ...
    if (!ok) {
        QMessageBox::warning(
            this, tr("Error"),
            tr("Failed to launch protontricks. Make sure Protontricks is installed."));
    }
}
...
void GameDetailsDialog::launchWinetricks() {
    ...
    if (!ok) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Failed to launch winetricks. Make sure Winetricks is installed."));
    }
}
...
void GameDetailsDialog::launchRegedit() {
    ...
    if (!ok) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Failed to launch regedit. Make sure Wine is installed."));
    }
}
...
void GameDetailsDialog::updateGame() {
    ...
    if (!game_.isInstalled || game_.installPath.trimmed().isEmpty()) {
        QMessageBox::information(this, tr("Not installed"), tr("This game is not installed."));
        return;
    }
...
void GameDetailsDialog::repairGame() {
    ...
    QMessageBox::StandardButton reply =
        QMessageBox::question(this, tr("Repair Game"),
