...
class CLI {
  public:
    ...
    void login(const QString &username, const QString &password) {
        ...
        session_->loginWithPassword(
            username, password, [this](util::Result<api::AuthTokens> result) {
                if (result.isOk()) {
                    std::cout << "Login successful!" << std::endl;
                    app_->quit();
                } else {
                    std::cerr << "Login failed: " << result.errorMessage().toStdString()
                              << std::endl;
                    app_->exit(1);
                }
            });
    ...
    void listGames() {
        ...
        libraryService_->fetchLibrary(
            false, [this](util::Result<std::vector<api::GameInfo>> result) {
                if (result.isOk()) {
                    ...
                    std::cerr << "Failed to fetch library: " << result.errorMessage().toStdString()
                              << std::endl;
                    app_->exit(1);
                }
            });
        ...
        libraryService_->getGame(gameId, [this, installDir](util::Result<api::GameInfo> result) {
            ...
            installService_->installGame(
                ...
                [this](util::Result<QString> result) {
                    std::cout << std::endl;
                    if (result.isOk()) {
                        std::cout << "Installation complete: " << result.value().toStdString()
                                  << std::endl;
                        app_->quit();
                    } else {
                        std::cerr << "Installation failed: " << result.errorMessage().toStdString()
                                  << std::endl;
                        app_->exit(1);
                    }
                });
    ...
    void listRunners() {
        ...
        for (const auto &runner : runners) {
            std::cout << "  " << runner.name.toStdString() << " " << runner.version.toStdString()
                      << std::endl;
            std::cout << "    Path: " << runner.executablePath.toStdString() << std::endl;
            std::cout << "    Platform: ";
            switch (runner.supportedPlatform) {
            case runners::Platform::Windows:
                std::cout << "Windows";
                break;
            case runners::Platform::Linux:
                std::cout << "Linux";
                break;
            case runners::Platform::MacOS:
                std::cout << "macOS";
                break;
            case runners::Platform::DOS:
                std::cout << "DOS";
                break;
            default:
                std::cout << "Unknown";
            ...
            if (runner.requiresISATranslation) {
                ...
                case runners::Architecture::X86:
                    std::cout << "x86";
                    break;
                case runners::Architecture::X86_64:
                    std::cout << "x86_64";
                    break;
                case runners::Architecture::ARM:
                    std::cout << "ARM";
                    break;
                case runners::Architecture::ARM64:
                    std::cout << "ARM64";
                    break;
                default:
                    std::cout << "Unknown";
...
int main(int argc, char *argv[]) {
    ...
    parser.addPositionalArgument("command",
                                 "Command to execute: login, list, install, launch, runners");

    QCommandLineOption usernameOption(QStringList() << "u" << "username", "Username for login",
                                      "username");
    QCommandLineOption passwordOption(QStringList() << "p" << "password", "Password for login",
                                      "password");
    QCommandLineOption gameIdOption(QStringList() << "g" << "game", "Game ID", "gameId");
    QCommandLineOption installDirOption(QStringList() << "d" << "dir", "Installation directory",
                                        "dir");
