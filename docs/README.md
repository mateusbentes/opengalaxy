# OpenGalaxy Documentation

Complete documentation for OpenGalaxy - the open-source GOG Galaxy alternative.

## 📚 Documentation Index

### Getting Started
- **[Installation Guide](INSTALL.md)** - Install pre-built binaries or packages
- **[Quick Start](QUICKSTART.md)** - Get up and running in 5 minutes
- **[Build Instructions](BUILD.md)** - Build from source for all platforms

### User Guides
- **[Compatibility Guide](COMPATIBILITY_GUIDE.md)** - Running games on different architectures (Wine, Proton, ISA translation)
- **[Multilingual Guide](MULTILINGUAL_GUIDE.md)** - Language support and translation
- **[Data Locations](DATA_LOCATIONS.md)** - Where OpenGalaxy stores your data
- **[Troubleshooting](TROUBLESHOOTING.md)** - Common issues and solutions

### Developer Documentation
- **[Architecture](ARCHITECTURE.md)** - System design and component overview
- **[Contributing Guide](../CONTRIBUTING.md)** - How to contribute to the project
- **[API Reference](API.md)** - Core library API documentation (coming soon)

### Project Information
- **[README](../README.md)** - Project overview and features
- **[Changelog](CHANGELOG.md)** - Version history and changes
- **[Features Status](FEATURES_STATUS.md)** - Implementation status of features
- **[License](../LICENSE)** - Apache 2.0 license

## 🎯 Quick Links by Task

### I want to...

**Install OpenGalaxy**
→ [INSTALL.md](INSTALL.md) - Pre-built binaries for Linux, Windows, macOS

**Build from source**
→ [BUILD.md](BUILD.md) - Detailed build instructions for all platforms

**Run Windows games on Linux**
→ [COMPATIBILITY_GUIDE.md](COMPATIBILITY_GUIDE.md) - Wine, Proton, and ISA translation setup

**Fix a problem**
→ [TROUBLESHOOTING.md](TROUBLESHOOTING.md) - Common issues and solutions

**Change the language**
→ [MULTILINGUAL_GUIDE.md](MULTILINGUAL_GUIDE.md) - Language settings and translation

**Find my game saves**
→ [DATA_LOCATIONS.md](DATA_LOCATIONS.md) - Data storage locations

**Contribute code**
→ [../CONTRIBUTING.md](../CONTRIBUTING.md) - Development setup and guidelines

**Understand the codebase**
→ [ARCHITECTURE.md](ARCHITECTURE.md) - System architecture and design

## 📖 Documentation by Audience

### End Users
1. [INSTALL.md](INSTALL.md) - Installation
2. [QUICKSTART.md](QUICKSTART.md) - First steps
3. [COMPATIBILITY_GUIDE.md](COMPATIBILITY_GUIDE.md) - Game compatibility
4. [TROUBLESHOOTING.md](TROUBLESHOOTING.md) - Problem solving
5. [DATA_LOCATIONS.md](DATA_LOCATIONS.md) - Data management

### Developers
1. [BUILD.md](BUILD.md) - Building the project
2. [ARCHITECTURE.md](ARCHITECTURE.md) - System design
3. [../CONTRIBUTING.md](../CONTRIBUTING.md) - Contribution guidelines
4. [API.md](API.md) - API reference (coming soon)

### Translators
1. [MULTILINGUAL_GUIDE.md](MULTILINGUAL_GUIDE.md) - Translation system
2. [../CONTRIBUTING.md](../CONTRIBUTING.md) - How to contribute translations

### System Administrators
1. [INSTALL.md](INSTALL.md) - Deployment options
2. [DATA_LOCATIONS.md](DATA_LOCATIONS.md) - Data storage
3. [BUILD.md](BUILD.md) - Building for distribution

## 🔍 Finding Information

### Search Tips
- Use your browser's search (Ctrl+F) within documents
- Check the [Troubleshooting Guide](TROUBLESHOOTING.md) first for issues
- See [Features Status](FEATURES_STATUS.md) for implementation details
- Read [Changelog](CHANGELOG.md) for recent changes

### External Resources
- **GitHub Issues**: https://github.com/mateusbentes/opengalaxy/issues
- **Discussions**: https://github.com/mateusbentes/opengalaxy/discussions
- **GOG API Docs**: https://gogapidocs.readthedocs.io/
- **Qt6 Documentation**: https://doc.qt.io/qt-6/

## 📝 Documentation Standards

All documentation follows these principles:
- **Clear and concise** - Easy to understand
- **Up-to-date** - Reflects current version
- **Well-organized** - Logical structure
- **Searchable** - Good headings and keywords
- **Practical** - Includes examples and commands

## 🤝 Contributing to Documentation

Found an error or want to improve the docs?

1. **Small fixes**: Edit directly on GitHub
2. **Large changes**: Follow [../CONTRIBUTING.md](../CONTRIBUTING.md)
3. **New guides**: Discuss in GitHub Discussions first

### Documentation Guidelines
- Use Markdown format
- Include code examples
- Add screenshots for UI features
- Keep language simple and clear
- Test all commands before documenting

## 📅 Last Updated

This documentation index was last updated: **2026-01-23**

For the latest version, see: https://github.com/mateusbentes/opengalaxy

---

**Need help?** Open an issue or discussion on GitHub!
